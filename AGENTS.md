...
| `.ap` | aiplaylist | `.ap lagu galau indo viral` |
| `.roast` | roast | `.roast` |
| `.chat` | chat | `.chat puisi tentang hujan --puisi` (owner only `--personality`) |
| `.limit` | ai-limit | `.limit` (cek sisa request) |
| `.ais` | ai-set | `.ais model MiniMax-M3` (owner only) |
...
## AI Features (Phase A-E)

Bot ini punya sistem AI yang udah di-upgrade dengan multi-provider (NVIDIA + TokenRouter), 13 personality, conversation memory, dan reliability layer.

### AI Commands

| Slash | Prefix | Access | Description |
|-------|--------|--------|-------------|
| `/chat prompt:<text> [personality:<nama>]` | `.chat <text> [--<personality>]` | Owner + whitelist | Ngobrol sama AI. 13 personality (auto-detect, owner bisa force). Reply pesan bot untuk lanjut (10 min TTL). |
| `/aiplaylist query:<tema>` | `.ap <tema>` | All + limit | AI bikin playlist dari tema/mood. Makan kuota (1 slot). |
| `/roast` | `.roast` | All + limit | AI roast lagu yang lagi diputar. Makan kuota (1 slot). |
| `/ai-limit` | `.limit` | All | Cek sisa request AI (ephemeral, gak makan quota). |
| `/ai-set <subcommand>` | `.ais <subcommand>` | Owner only | Atur setting global AI. |

### 13 Personalities

`general`, `roast-galau`, `roast-pemerintah`, `romantis`, `puisi`, `motivator`, `coding-helper`, `storyteller`, `debate`, `gym-buddy`, `chef`, `game-strategist`, `joker`. Tone: casual `lo/gue` style, bukan formal AI assistant.

### Owner Personality Override (one-shot)

Owner bisa force personality per-chat:
- **Slash:** `/chat prompt:tulis puisi personality:puisi`
- **Prefix:** `.chat tulis puisi tentang hujan --puisi`

Personality cuma berlaku untuk 1 chat itu, gak persist ke reply berikutnya.

### `/ai-set` Subcommands

| Subcommand | Usage | Description |
|-----------|-------|-------------|
| `model` | `model <MiniMax-M3\|llama-3.3-70b>` | Switch model. Provider auto-set. |
| `limit` | `limit <angka>` | Global per-user hourly limit (default 5). |
| `userlimit` | `userlimit <set\|remove\|list> [@user] [value]` | Per-user override limit. |
| `bonus` | `bonus <set\|add\|remove\|list> [@user] [value]` | Per-user bonus/penalty (bisa negatif). |
| `reset-limit` | `reset-limit [@user\|all]` | Manual reset counter window. |
| `whitelist` | `whitelist <add\|remove\|list> [@user]` | Manage user yang boleh pakai /chat. |
| `memory` | `memory <view\|set\|clear\|global> [@user] [field] [value]` | AI memory management. Fields: nickname, mood, genre, artist, interests. |
| `fallback` | `fallback <on\|off>` | Auto-fallback ke provider alternatif on 5xx. |
| `cache` | `cache <stats\|clear>` | Manage prompt cache. |
| `limits` | `limits` | Monitor semua user yang lagi aktif. |
| `view` | `view` | Lihat semua setting. |

### Rate Limit Logic

`effective limit = userLimits[userId] || (userHourlyLimit + userBonuses[userId])`

Owner bypass (unlimited). Limit window: 1 jam rolling. Cek sisa: `/ai-limit` atau `.limit`.

### Data Persistence

- `data/aiSettings.json` — global AI config (model, limit, whitelist, dll)
- `data/aiMemory.json` — per-user profiles + global notes
- `data/guildSettings.json` — per-guild music settings (existing)